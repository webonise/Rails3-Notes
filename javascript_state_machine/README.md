...
Description
========
Javascript state machine can be used for maintaining states in javascript. Too often, the state of an javascript object is kept by creating multiple boolean attributes and deciding how to behave based on the values. This can become cumbersome and difficult to maintain when the complexity of your object starts to increase.Javascript state machine simplifies this design by introducing the various parts of a real state machine, including states, events, transitions and callbacks.
...
In its simplest form, create a standalone state machine using:
    ...
    var fsm = StateMachine.create({ /* here fsm stands for finite state machine */
      initial: 'green',
      events: [
...
define a custom `error` handler:
    ...
    alert(fsm.calm()); // "event calm :- event not allowed in current state green"
